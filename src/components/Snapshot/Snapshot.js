...
function VolumeHead(props) {
  ...
  return (
    props ? (<Tooltip placement="right"
      ...
      title={<div>
      <p className="snapshot-name">{t('snapshot.tooltip.name')}: {props.name}</p>
      <p className="snapshot-created">{t('snapshot.tooltip.created')}: {props.created}</p>
      <p className="snapshot-name">{t('snapshot.tooltip.size')}: {formatMib(props.size)}</p>
  ...
  return (
    ...
    >
      <Tooltip placement="right"
        autoAdjustOverflow={false}
        title={<div>
        <p className="snapshot-name">{t('snapshot.tooltip.name')}: {snapshotObject.name}</p>
        <p className="snapshot-created">{t('snapshot.tooltip.created')}: {snapshotObject.created}</p>
        <p className="snapshot-name">{t('snapshot.tooltip.size')}: {backupStatusObject && backupStatusObject.progress === 100 ? formatMib(backupStatusObject.size) : formatMib(snapshotObject.size)}</p>
