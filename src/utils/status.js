...
export function statusUpgradingEngine(volume, t) {
  if (volume && volume.image && volume.currentImage && volume.image !== volume.currentImage) {
    return (<Tooltip title={t('volumeStatus.upgradingEngineTooltip', { from: volume.currentImage, to: volume.image })}><Icon style={{ marginRight: 5 }} type="sync" spin /></Tooltip>)
  }
  return ''
}
