    ...
    <ModalBlur {...modalOpts}>
      <Form layout="horizontal">
        ...
        {item.secretNamespace && (
         ...
         <FormItem label={t('restoreBackupBackingImageModal.form.dataEngine.label')} hasFeedback {...formItemLayout}>
            ...
            rules: [
              {
                validator: (_rule, value, callback) => {
                  if (value === 'v1' && !v1DataEngineEnabled) {
                    callback(t('restoreBackupBackingImageModal.form.dataEngine.v1NotEnabled'))
                  } else if (value === 'v2' && !v2DataEngineEnabled) {
                    callback(t('restoreBackupBackingImageModal.form.dataEngine.v2NotEnabled'))
                  }
                  callback()
                },
              },
