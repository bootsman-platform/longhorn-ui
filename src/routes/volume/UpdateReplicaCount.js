  ...
  return (
    <ModalBlur {...modalOpts}>
      <Form layout="horizontal">
        <FormItem label={t('updateReplicaCount.fields.numberOfReplicas')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('replicaCount', {
            initialValue: item.numberOfReplicas,
            rules: [
              {
                required: true,
                message: t('updateReplicaCount.validation.replicasRequired'),
              },
              {
                validator: (rule, value, callback) => {
                  if (value < 1 || value > 20 || !/^\d+$/.test(value)) {
                    callback(t('updateReplicaCount.validation.replicasRange'))
                  } else {
                    callback()
                  }
                },
