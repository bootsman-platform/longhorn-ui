  ...
  return (
    <ModalBlur {...modalOpts}>
      <Form layout="horizontal">
        <FormItem label={t('updateBulkReplicaCount.fields.numberOfReplicas')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('replicaCount', {
            initialValue,
            rules: [
              {
                required: true,
                message: t('updateBulkReplicaCount.validation.replicasRequired'),
              },
              {
                validator: (rule, value, callback) => {
                  if (value < 1 || value > 20 || !/^\d+$/.test(value)) {
                    callback(t('updateBulkReplicaCount.validation.replicasRange'))
                  } else {
                    callback()
                  }
                },
