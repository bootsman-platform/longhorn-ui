  ...
  return (
    <ModalBlur {...modalOpts}>
      ...
      <Form layout="horizontal" {...formItemLayout} style={{ display: 'flex' }}>
        <FormItem label={t('expansionVolumeSizeModal.fields.size')} style={{ flex: 0.7 }}>
          {getFieldDecorator('size', {
            initialValue: minValue,
            rules: [
              {
                required: true,
                message: t('expansionVolumeSizeModal.validation.sizeRequired'),
              }, {
                validator: (rule, value, callback) => {
                  ...
                  if (value < 0 || value > 65536) {
                    callback(t('expansionVolumeSizeModal.validation.sizeRange'))
