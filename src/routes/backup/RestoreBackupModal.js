  ...
  return (
    <ModalBlur {...modalOpts}>
      <Form layout="horizontal">
        ...
        <FormItem label={t('restoreBackupModal.form.dataEngine.label')} hasFeedback {...formItemLayout}>
              ...
              },
              {
                validator: (rule, value, callback) => {
                  if (value === 'v1' && !v1DataEngineEnabled) {
                    callback(t('restoreBackupModal.form.dataEngine.v1NotEnabled'))
                  } else if (value === 'v2' && !v2DataEngineEnabled) {
                    callback(t('restoreBackupModal.form.dataEngine.v2NotEnabled'))
                  }
                  callback()
                },
              },
        ...
        <FormItem label={t('restoreBackupModal.form.numberOfReplicas.label')} hasFeedback {...formItemLayout}>
        ...
        <FormItem label={t('restoreBackupModal.form.accessMode.label')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('accessMode', {
            initialValue: item.accessMode,
          })(<Select>
            <Option key={'ReadWriteOnce'} value={'rwo'}>{t('restoreBackupModal.form.accessMode.options.readWriteOnce')}</Option>
            <Option key={'ReadWriteOncePod'} value={'rwop'}>{t('restoreBackupModal.form.accessMode.options.readWriteOncePod')}</Option>
            <Option key={'ReadWriteMany'} value={'rwx'}>{t('restoreBackupModal.form.accessMode.options.readWriteMany')}</Option>
          </Select>)}
        </FormItem>
        <FormItem label={t('restoreBackupModal.form.backingImage.label')} hasFeedback {...formItemLayout}>
        ...
        <FormItem label={t('restoreBackupModal.form.encrypted.label')} {...formItemLayout}>
        ...
        <Spin spinning={tagsLoading}>
          <FormItem label={t('restoreBackupModal.form.nodeTag.label')} hasFeedback {...formItemLayout}>
        ...
        <Spin spinning={tagsLoading}>
          <FormItem label={t('restoreBackupModal.form.diskTag.label')} hasFeedback {...formItemLayout}>
        ...
        <FormItem label={t('restoreBackupModal.form.backupBlockSize.label')} hasFeedback {...formItemLayout}>
