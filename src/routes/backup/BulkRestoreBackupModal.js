  ...
  return (
    <ModalBlur {...modalOpts}>
      ...
      <Form key={currentTab} layout="horizontal">
        ...
        <FormItem label={t('bulkRestoreBackupModal.form.dataEngine.label')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('dataEngine', {
            initialValue: item.dataEngine,
            rules: [
              {
                required: true,
                message: t('bulkRestoreBackupModal.form.dataEngine.validationMessage'),
              },
              {
                validator: (_rule, value, callback) => {
                  if (value === 'v1' && !v1DataEngineEnabled) {
                    callback(t('bulkRestoreBackupModal.form.dataEngine.v1NotEnabled'))
                  } else if (value === 'v2' && !v2DataEngineEnabled) {
                    callback(t('bulkRestoreBackupModal.form.dataEngine.v2NotEnabled'))
                  }
                  callback()
                },
              },
        ...
        <FormItem label={t('bulkRestoreBackupModal.form.numberOfReplicas.label')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('numberOfReplicas', {
            initialValue: item.numberOfReplicas,
            rules: [
              {
                required: true,
                message: t('bulkRestoreBackupModal.form.numberOfReplicas.validationMessage'),
              },
            ],
          })(<InputNumber min={1} max={10} onChange={handleReplicasNumberChange} />)
          }
        </FormItem>
        <FormItem label={t('bulkRestoreBackupModal.form.accessMode.label')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('accessMode', {
            initialValue: item.accessMode,
          })(<Select onSelect={handleAccessModeChange}>
            <Option key={'ReadWriteOnce'} value={'rwo'}>{t('bulkRestoreBackupModal.form.accessMode.options.readWriteOnce')}</Option>
            <Option key={'ReadWriteOncePod'} value={'rwop'}>{t('bulkRestoreBackupModal.form.accessMode.options.readWriteOncePod')}</Option>
            <Option key={'ReadWriteMany'} value={'rwx'}>{t('bulkRestoreBackupModal.form.accessMode.options.readWriteMany')}</Option>
          </Select>)}
        </FormItem>
        ...
        <FormItem label={t('bulkRestoreBackupModal.form.backingImage.label')} hasFeedback {...formItemLayout}>
        ...
        <FormItem label={t('bulkRestoreBackupModal.form.encrypted.label')} {...formItemLayout}>
          {getFieldDecorator('encrypted', {
            valuePropName: 'checked',
            initialValue: item.encrypted || false,
          })(<Checkbox onChange={handleEncryptedCheck} />)}
        </FormItem>
        <FormItem label={t('bulkRestoreBackupModal.form.restoreVolumeRecurringJob.label')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('restoreVolumeRecurringJob', {
            initialValue: 'ignored',
          })(<Select onSelect={handleRecurringJobChange}>
            <Option key={'enabled'} value={'enabled'}>{t('bulkRestoreBackupModal.form.restoreVolumeRecurringJob.options.enabled')}</Option>
            <Option key={'disabled'} value={'disabled'}>{t('bulkRestoreBackupModal.form.restoreVolumeRecurringJob.options.disabled')}</Option>
            <Option key={'ignored'} value={'ignored'}>{t('bulkRestoreBackupModal.form.restoreVolumeRecurringJob.options.ignored')}</Option>
          </Select>)}
        </FormItem>
        <Spin spinning={tagsLoading}>
          <FormItem label={t('bulkRestoreBackupModal.form.nodeTag.label')} hasFeedback {...formItemLayout}>
        ...
        <Spin spinning={tagsLoading}>
          <FormItem label={t('bulkRestoreBackupModal.form.diskTag.label')} hasFeedback {...formItemLayout}>
        ...
        <FormItem label={t('bulkRestoreBackupModal.form.backupBlockSize.label')} hasFeedback {...formItemLayout}>
          {getFieldDecorator('backupBlockSize', {
            ...
            <Select>
              <Option key="ignored" value="0">{t('bulkRestoreBackupModal.form.backupBlockSize.options.ignored')}</Option>
