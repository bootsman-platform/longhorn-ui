          ...
          <div className={styles.control} style={{ lineHeight: '40px' }}>
            {disk && disk.conditions && Object.keys(disk.conditions).map((key) => {
              let title = (<div>
                ...
                {disk.conditions[key] && disk.conditions[key].status && disk.conditions[key].status ? <div style={{ marginBottom: 5 }}>{t('editableDisk.conditions.status')}: {disk.conditions[key].status}</div> : ''}
              </div>)
