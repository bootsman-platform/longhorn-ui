            ...
            <div className={styles.control} style={{ lineHeight: '40px' }}>
              {node && node.conditions && Object.keys(node.conditions).map((key) => {
                let title = (<div>
                  ...
                  {node.conditions[key] && node.conditions[key].status && node.conditions[key].status ? <div style={{ marginBottom: 5 }}>{t('editableDisk.conditions.status')}: {node.conditions[key].status}</div> : ''}
                </div>)
