...
function Host({ host, volume, setting, loading, dispatch, location, t }) {
  ...
  const hostFilterProps = {
    ...
    fieldOption: [
      { value: 'name', name: t('resource.node.filter.name') },
      { value: 'status', name: t('resource.node.filter.status') },
      { value: 'NodeTag', name: t('resource.node.filter.nodeTag') },
      { value: 'DiskTag', name: t('resource.node.filter.diskTag') },
    ],
    expandAll() {
